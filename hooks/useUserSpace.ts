...
const useUserSpace = () => {
	...
	const userFollowedSpaceIds = useMemo(() => {
		const ids = (userRoles || [])
			.filter((role) => role?.resourceId && role.roleId === followerRoleId && role?.source?.toLocaleLowerCase() === 'space')
			.map((role) => role?.resourceId);
		return new Set(ids);
	...
	return {
		...
		isUserSpaceLoading:
			isUserRoleLoading || isUserSpaceAndEventLoading,
		isUserSpaceFetched:
			isUserRoleFetched && isUserSpaceAndEventFetched,
	};
}
