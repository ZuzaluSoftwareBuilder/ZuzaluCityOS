...
const MobileNav: React.FC<MobileNavProps> = ({
  ...
  return (
    <>
      <div className="flex items-center gap-[10px] bg-[rgba(34,34,34,0.90)] backdrop-blur-[10px] py-[10px]">
        ...
        >
          ...
          <div className={`flex items-center ${filterCount > 0 ? 'opacity-100' : 'opacity-50'} gap-[5px]`}>
            <Funnel size={16} weight="fill" color="#FFFFFF" />
      ...
      {/* filter panel */}
      {isFilterOpen && (
        <div className="flex justify-between items-center gap-[10px] bg-[rgba(34,34,34,0.90)] backdrop-blur-[10px] py-[10px] px-[20px]">
          <div className="flex gap-[10px]">
            {/* location filter */}
            <Dropdown isOpen={isLocationDropdownOpen} onOpenChange={(open) => setIsLocationDropdownOpen(open)}>
              <DropdownTrigger>
                <HeroButton 
                  className={`flex items-center gap-[5px] py-[5px] px-[10px] rounded-[5px] ${selectedLocation !== 'anywhere' ? 'bg-[#4A4A4A]' : 'bg-[#363636]'}`}
                >
                  <span className="text-[14px] text-white">Location</span>
                  {selectedLocation !== 'anywhere' && (
                    <span className="text-[14px] text-[#7DFFD1]">1</span>
                  )}
                </HeroButton>
              </DropdownTrigger>
              <DropdownMenu 
              ...
              >
                {locations.map((location) => (
                  <DropdownItem 
                    key={location.key} 
                    className={`py-[6px] px-[10px] rounded-[8px] text-white text-[13px] font-[500] ${selectedLocation === location.key ? 'bg-[rgba(255,255,255,0.1)]' : 'bg-[rgba(255,255,255,0.05)]'}`}
                  >
                    {location.label}
            ...
            <Dropdown isOpen={isTimeDropdownOpen} onOpenChange={(open) => setIsTimeDropdownOpen(open)}>
              <DropdownTrigger>
                <HeroButton 
                  className={`flex items-center gap-[5px] py-[5px] px-[10px] rounded-[5px] ${timeFilter !== 'upcoming' ? 'bg-[#4A4A4A]' : 'bg-[#363636]'}`}
                >
          ...
          >
            <DropdownTrigger>
              <HeroButton 
                className={`flex items-center justify-center w-[40px] h-[32px] rounded-[5px] ${selectedDate ? 'bg-[#4A4A4A]' : 'bg-[#363636]'} p-[4px_8px] relative`}
              >
                <CalendarBlank size={20} weight="fill" color="#FFFFFF" />
                {selectedDate && (
                  <span className="absolute top-[-5px] right-[-5px] text-[14px] text-[#7DFFD1]">1</span>
                )}
              </HeroButton>
            </DropdownTrigger>
            <DropdownMenu 
              aria-label="Calendar"
              className="bg-[rgba(34,34,34,0.8)] backdrop-blur-[24px] border border-[rgba(255,255,255,0.1)] rounded-[10px] p-[14px] min-w-[280px]"
              closeOnSelect={false}
            >
              <DropdownItem key="calendar-item" className="p-0 focus:outline-none">
                <div className="w-full">
                  <p className="py-[10px] px-[10px] text-[16px] font-[700] leading-[1.2] border-b border-b-w-10 text-white mb-[10px]">
                    Select Date
                  </p>
                  <Calendar
                    value={selectedDate}
