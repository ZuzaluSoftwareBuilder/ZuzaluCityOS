...
export const AvatarUpload: React.FC<AvatarUploadProps> = ({
  ...
  const avatarOverlay = isLoading ? (
    ...
    isHovering && localUrl && (
      <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-40  transition-opacity duration-200 z-[10]">
        <PhotoIcon className="w-6 h-6 text-white opacity-50" />
      </div>
    )
  );
