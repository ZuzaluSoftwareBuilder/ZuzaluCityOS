...
import { InvitationService, InvitationError } from '@/services/invitation/InvitationService';
...
export const withInvitationValidation = (
  ...
  return async (req: NextRequest, sessionData: SessionData): Promise<NextResponse> => {
    try {
      ...
      await invitationService.validateInvitation(invitation, options.expectedStatus);

      if (options.validateInvitee) {
        invitationService.validatePermission(invitation, sessionData.operatorId, true);
      }

      if (options.validateInviter) {
        invitationService.validatePermission(invitation, sessionData.operatorId, false);
      }
      ...
      if (error instanceof InvitationError) {
        return createErrorResponse(error.message, 400, { code: error.code, details: error.details });
      }
